...
class CIFAR_PAQ_COMP:
	...
	def average_weights(self, all_weights):
		# all_weights is a list of lists: each element is a list of [layer_weights, layer_biases]
		avg_weights = []

		# Iterate over each layer's weights and biases
		for layer_index in range(len(all_weights[0])):
			# Collect all client weights and biases for this layer
			layer_weights = np.mean([client[layer_index][0] for client in all_weights], axis=0)
			layer_biases = np.mean([client[layer_index][1] for client in all_weights], axis=0)

			# Convert averaged weights and biases to PyTorch tensors
			avg_weights.append([
				torch.from_numpy(layer_weights.astype(np.float32)),
				torch.from_numpy(layer_biases.astype(np.float32))
			])
